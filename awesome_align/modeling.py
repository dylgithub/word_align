...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def get_aligned_word(self, inputs_src, inputs_tgt, bpe2word_map_src, bpe2word_map_tgt, device, src_len, tgt_len,
        ...
        if word_aligns is None:
            ...
            with torch.no_grad():
                # PAD_ID填充的token_id(0)
                # attention_mask记录当前token是不是填充的token
                # outputs_src[batch_size,batch_size中最大的句子长度，768]
                outputs_src = self.bert(
                    inputs_src,
                    align_layer=align_layer,
                ...
                # [batch_size,1,句子1的真实长度+2,句子2的真实长度+2]
            ...
            # b2w_src中存的是每个token到词的映射[0,1,1,2,2,3,3]意思是第三个token属于第一个词
